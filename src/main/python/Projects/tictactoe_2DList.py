...
#BOARD UPDATERS START
    def clear_board(self):
        ...
        rows = 3
        columns = 3
        board_values = [[" " for row in range(rows)] for column in range(columns)]
        turn_sum = 0
    ...
    def update_board(self, gamemode, board_values, position, turn="user", turn_sum=0, available_cells=list(range(9))): 
        ...
        if board_values[position_coords[0], position_coords[1]] == " ":
            if turn == "user":
                board_values[position_coords[0], position_coords[1]] = "X"
                turn_sum += 1
            elif turn == "ai":
                board_values[position_coords[0], position_coords[1]] = "O"   
            available_cells[position_coords[0]].pop(position_coords[1])
        else:
    ...
    def print_board(self, board_values : list, turn_sum):
        ...
        while x < 3:
            ...
            print("     |     |     ") #Row 1
            print("", board_values[x][0], "|", board_values[x][1], "|", board_values[x][2], sep="  ") #Row 2
